#IoT coreにNo1QAよりjsonがsubscribeされたらeventが発生して呼び出されるLambda関数
...
from time_list_gen import startend_list_generator
...
def exclude_downtime(now, str_starttime, today, lunch, PM_SP, even_mtg, ely_end1, ely_end2):
    ...
    t_list, check_point = startend_list_generator(today, start_time, lunch, PM_SP, even_mtg, ely_end1, ely_end2)

    #設備稼働中の不稼働時間の折り込み
    for i in range(len(t_list)):
        for j in range(len(check_point)):
            if (t_list[i][0] <= now < t_list[i][1]) and (check_point[j][0] <= now < check_point[j][1]):
                str_now = (now - t_list[i][2] - check_point[j][5]*timedelta(seconds=600)).strftime('%Y-%m-%d %H:%M:%S') #t_list[i][2]:定常downtime、check_point[j][5]:ドレス回数

    #設備停止中の不稼働時間の折り込み
    for i in range(len(t_list)):
        for j in range(len(check_point)):
            #休憩時間以外のドレッシング
            if (t_list[i][0] <= now < t_list[i][1]) and (check_point[j][1] <= now < check_point[j][2]):
                    str_now = (check_point[j][1] - t_list[i][2] - check_point[j][5]*timedelta(seconds=600)).strftime('%Y-%m-%d %H:%M:%S') #t_list[i][2]:定常downtime、check_point[j][1]:ドレス開始時間
            #休憩時間
            if i < len(t_list)-1:
                if (t_list[i][1] <= now < t_list[i+1][0]) and (check_point[j][2] == t_list[i][1]):
                    str_now = (check_point[j][1] - t_list[i][2] - check_point[j][5]*timedelta(seconds=600)).strftime('%Y-%m-%d %H:%M:%S') #t_list[i][2]:定常downtime、check_point[j][1]:ドレス開始時間
            #休憩時間をはさんだドレッシング
            if (t_list[i][0] <= now < t_list[i][1]) and ((check_point[j][0] == check_point[j][1]) and (check_point[j][1] <= now < check_point[j][2])):
                    str_now = (check_point[j-1][1] - t_list[i-1][2] - check_point[j-1][5]*timedelta(seconds=600)).strftime('%Y-%m-%d %H:%M:%S') #t_list[i][2]:定常downtime、check_point[j][1]:ドレス開始時間
...
def lambda_handler(event, context):
    ...
    #lunch = get_dynamo['Item']['lunch_support_BUFF1']#1S昼稼働
    #PM_SP = get_dynamo['Item']['PMstop1S_BUFF1']#1S PM停止(作業者半休等)
    #even_mtg = get_dynamo['Item']['even_meeting_BUFF1']#全体夕礼
    #ely_end1 = get_dynamo['Item']['16:40_end_BUFF1']#1S 16:40終了
    #ely_end2 = get_dynamo['Item']['17:40_end_BUFF1']#1S 17:40終了
